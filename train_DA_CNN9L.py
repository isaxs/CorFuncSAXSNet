...
class NineLayerConvNet(nn.Module):
    def __init__(self):
        super(NineLayerConvNet, self).__init__()
        self.in_channels = 2
        # 第1层卷积
        self.conv1 = nn.Conv1d(self.in_channels, 64, kernel_size=3, stride=1, padding=1)
        ...
        # 第7层卷积
        self.conv7 = nn.Conv1d(512, 512, kernel_size=3, stride=1, padding=1)
...
def train_model(model, train_loader, criterion, optimizer, num_epochs=10, patience=10, train_loss_threshold=0.25,
                test_loss_threshold=0.35):
    best_loss = float('inf')
    counter = 0
    model.train()
     # 用于记录训练集损失、测试集损失和训练轮次
    train_losses = []
    test_losses = []
    epochs = []
    for epoch in range(num_epochs):
        ...
        if train_loss < train_loss_threshold and test_loss < test_loss_threshold:
            ...
            torch.save(model.state_dict(), 'model_CNN9L_weights.pth')
            break
        ...
        loss_data = {
            ...
            'epochs': epochs}
        with open('loss_data_CNN9L.pkl', 'wb') as f:
...
model = NineLayerConvNet().to(device)  # 修改为 NineLayerConvNet
...
model_graph = draw_graph(
    ...
    graph_name='CNN9L_Arch',
    directory='model_graph',  # 指定输出目录
    save_graph=True,        # 自动保存图表
    filename='CNN9L_Architecture',  # 文件名
)

# 手动保存高分辨率图片（可选，如果自动保存不生效）
model_graph.visual_graph.render(
    format='svg',
    filename='CNN9L_Architecture',
    cleanup=True,
)
...
# 加载模型权重
model.load_state_dict(torch.load("model_CNN9L_weights.pth"))
# 将模型设置为评估模式
model.eval()
model.to(device)
...
# 保存到不同文件
np.savetxt('CNN9L_pred_param1.dat', np.column_stack((true_param1, pred_param1)), fmt='%f', header='True Param1  Predicted Param1')
np.savetxt('CNN9L_pred_param2.dat', np.column_stack((true_param2, pred_param2)), fmt='%f', header='True Param2  Predicted Param2')
...
results_df.to_csv('CNN9L_full_predictions.csv', index=False)
